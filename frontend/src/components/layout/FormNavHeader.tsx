...
export interface FormNavHeaderProps {
  form: Form;
...
export const FormNavHeader: React.FC<FormNavHeaderProps> = ({
  form,
  saveStatus,
  onTitleChange,
  onPublish,
  ...
  const handleTitleSubmit = () => {
    ...
    if (onTitleChange && titleText.trim() && titleText !== form.title) {
      onTitleChange(titleText.trim());
    }
  };

  const navTabs = [
    { label: "Builder", href: `/forms/${form.id}/edit` },
    { label: "Results & Analytics", href: `/forms/${form.id}/results` },
    { label: "Settings", href: `/forms/${form.id}/settings` },
  ];
  ...
  return (
    <header className="sticky top-0 z-30 bg-white border-b border-slate-200 shadow-2xs">
      <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          {isEditingTitle && onTitleChange ? (
            ...
            <h1
              ...
              className={`text-sm font-bold text-slate-900 truncate ${
                onTitleChange ? "hover:text-blue-600 cursor-pointer" : ""
              } transition-colors`}
              title={onTitleChange ? "Click to rename form title" : form.title}
            >
              {form.title}
            </h1>
          )}
        ...
        {/* Center Section: Context Sub-navigation Tabs */}
        ...
        {/* Right Section: Preview & Publish Actions */}
